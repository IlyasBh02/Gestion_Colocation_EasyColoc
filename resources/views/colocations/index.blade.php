    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @forelse($colocations as $index => $colocation)
                    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg border border-gray-100 dark:border-gray-700 hover-lift animate-fadeInUp" style="animation-delay: {{ $index * 0.1 }}s;">
                        <div class="p-6">
                            <div class="flex justify-between items-start mb-4">
                                <h3 class="text-xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">{{ $colocation->name }}</h3>
                                <span class="px-2 py-1 text-xs font-semibold rounded-full bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-900 dark:to-purple-900 text-indigo-700 dark:text-indigo-300 border border-indigo-100 dark:border-indigo-800">
                            ...
                            <div class="mt-6 flex items-center justify-between">
                                ...
                                @can('update', $colocation)
                                    <div class="flex space-x-2">
                                        <a href="{{ route('colocations.edit', $colocation) }}" class="text-yellow-600 dark:text-yellow-400 hover:text-yellow-900 dark:hover:text-yellow-300 transform hover:scale-110 transition-transform">
                                        ...
                                        <form action="{{ route('colocations.destroy', $colocation) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this colocation?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-red-600 dark:text-red-400 hover:text-red-900 dark:hover:text-red-300 transform hover:scale-110 transition-transform">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
